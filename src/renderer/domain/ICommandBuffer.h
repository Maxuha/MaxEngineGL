...
namespace Rendering {
    ...
    public:
        ...
        virtual void BeginRenderPass(const AttachmentDescription& attachment, GLuint fboId, int w, int h,
                                     Color clearColor) = 0;
        ...
        virtual void BindVertexBuffer(BufferId &buffer) = 0;

        virtual void BindIndexBuffer(BufferId &buffer) = 0;

        virtual void BindUniformBuffer(uint32_t bindingPoint, const BufferId &buffer) = 0;

        virtual void BindResourceSet(uint32_t setIndex, const IResourceSet &resourceSet) = 0;

        virtual void PushConstants(const std::string &key, Matrix4x4 mat) = 0;
